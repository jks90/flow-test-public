...
docker run -d --add-host=host.docker.internal:host-gateway \
  ...
  --name flow juankanh/flow-app:4.25.5
```
...
```bash
docker pull juankanh/flow-app:4.25.5
docker stop flow && docker rm flow
docker run -d ... juankanh/flow-app:4.25.5   # mismo run de siempre
```
