...
docker run -d --add-host=host.docker.internal:host-gateway \
  ...
  --name flow juankanh/flow-app:4.29.0
```
...
```bash
docker pull juankanh/flow-app:4.29.0
docker stop flow && docker rm flow
docker run -d ... juankanh/flow-app:4.29.0   # mismo run de siempre
```
