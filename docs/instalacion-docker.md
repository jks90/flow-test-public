...
docker run -d --add-host=host.docker.internal:host-gateway \
  ...
  --name flow juankanh/flow-app:4.25.4
```
...
```bash
docker pull juankanh/flow-app:4.25.4
docker stop flow && docker rm flow
docker run -d ... juankanh/flow-app:4.25.4   # mismo run de siempre
```
