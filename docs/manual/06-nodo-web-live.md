...
- Desde la **4.7.0** se teclea **directamente sobre la vista**: clica un campo y escribe (Enter/Tab/flechas incluidas); **Ctrl+V pega el portapapeles** (el OTP del correo) y Ctrl+letra llega a la web como atajo. La vista marca el foco con «teclado → web». ⚠ Lo tecleado viaja en claro a tu server local y no se loguea; para contraseñas usa mejor Ctrl+V.
...
- Desde la **4.8.0** el nodo tiene pestañas **Consola** y **Network**: con la sesión Live abierta ves los `console.*` y errores de la página, y todo su tráfico (método, URL, status, tipo, duración — fallos `net::ERR_*` incluidos). Solo método/URL/status: nunca headers ni cuerpos. Contadores en vivo en las pestañas y botón Limpiar. Perfecto para depurar por qué un paso no hace la llamada que esperabas antes de capturarlo.
